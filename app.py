...
def resolve_ticker(x):
    mapping = {
        ...
        "citi":"C","citigroup":"C","sap":"SAP.DE",
        "bmw":"BMW.DE","siemens":"SIE.DE"
    }
...
if ticker:
    ...
    # =========================
    # PROFILE
col1, col2 = st.columns([1,2]) with col1: st.markdown(f""" <div class="card"> <h3>{name}</h3> <p><b>Sector:</b> {sector}</p> <p><b>Country:</b> {country}</p> </div> """, unsafe_allow_html=True) with col2: st.markdown(f""" <div class="card" style=" max-height:140px; overflow-y:auto; padding:18px; line-height:1.6; "> <p> {info.get("longBusinessSummary","No data")} </p> </div> """, unsafe_allow_html=True) add country flag also
    # =========================
    # CALCULATIONS
    # =========================
    ...
    def kpi(title,value,level):
        if level=="Low":
            color=GREEN; bg="#22C55E20"
        elif level=="Medium":
            color=ORANGE; bg="#F59E0B20"
        else:
            color=RED; bg="#EF444420"
    ...
    with col_left:
        ...
        fig.add_trace(go.Scatter(x=data.index,y=data["MA30"],name="MA30"))
    ...
    with col_right:
        ...
        st.markdown(kri("Control Effectiveness",control,GREEN),unsafe_allow_html=True)

    # =========================
    # 📋 DORA REGISTER (UNCHANGED)
    # =========================
    ...
    risk_df = pd.DataFrame({
        ...
        "Control Effectiveness": [
            "Needs Improvement","Effective","Needs Improvement","Effective","Needs Improvement"
        ],
        "Status": ["Open","Monitoring","Open","Mitigated","Monitoring"]
    })
    ...
    risk_df["Risk Level"] = risk_df["Risk Level"].apply(lambda x: color_tag(x, RED if x=="High" else ORANGE if x=="Medium" else GREEN))
    risk_df["Control Effectiveness"] = risk_df["Control Effectiveness"].apply(lambda x: color_tag(x, GREEN if x=="Effective" else ORANGE))
    risk_df["Status"] = risk_df["Status"].apply(lambda x: color_tag(x, RED if x=="Open" else ORANGE if x=="Monitoring" else GREEN))
